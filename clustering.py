...
if __name__ == "__main__":
    ...
    f = open("clustering_mmr_diff.txt", 'w')
    f.write('\n'.join(str(item) for item in total_DEBUG_delta_mmr[0]))
    f.close()

    f = open("clustering_waiting_time.txt", 'w')
    f.write('\n'.join(str(item) for item in total_DEBUG_waittime[0]))
    f.close()
